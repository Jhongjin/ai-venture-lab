...
| Data class | Default disposition | Owner | Notes |
| --- | --- | --- | --- |
| Authenticated write smoke workspace | Delete or archive after the test window | Operator | Keep only if it is intentionally reused as a disposable beta fixture. |
| Authenticated write smoke idea/package | Delete or archive after the test window | Operator | The record should have a recognizable smoke prefix and no real customer data. |
| RLS disposable workspaces | Keep as reusable fixtures until the next RLS policy or workspace access change | Operator | Delete only after deciding that future RLS reruns will use a new fixture pair. |
| RLS disposable Auth users | Keep as reusable fixtures while the matching workspace pair exists | Operator | Do not reuse primary operator, customer, or personal accounts. |
| Telemetry smoke events | Keep summary evidence; delete raw test events only with explicit DB cleanup approval | Operator | Event payloads must remain disposable and secret-free. |
| Smoke screenshots | Avoid by default; delete local sensitive screenshots after review | Operator | Do not store screenshots containing private workspace data in git. |
| Docs and phase evidence | Keep summary-only records | Agent | Never include credentials, raw payloads, cookies, sessions, signed URLs, or secret values. |
...
2. If the record is needed as a stable RLS fixture pair, keep it and label it as reusable.
3. If the record was created by authenticated write smoke and is no longer needed, delete or archive it after the test window.
4. If the record is a telemetry smoke event, keep only summary evidence in docs and delete raw events only after explicit user-approved DB cleanup.
5. If cleanup would require SQL, service-role access, Supabase dashboard mutation, Vercel env mutation, deploy, rollback, or external runtime mutation, record it as a user-owned cleanup action.
...
Disposition: kept_as_fixture | deleted | archived | user_owned_cleanup | not_applicable
Cleanup owner:
Secrets printed: no
