...
| --- | --- | --- |
| Public client config | `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `NEXT_PUBLIC_APP_URL`, `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` | Browser-safe config. The anon key relies on RLS and is not a service-role key. Stripe publishable keys are not checkout authority. |
| Server-only secrets | `SUPABASE_SERVICE_ROLE_KEY`, `TELEMETRY_INGEST_SECRET`, `OPENAI_API_KEY`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET` | Trusted server environments only. Never expose to browser bundles, screenshots, artifacts, chat, MCP packets, or Build Relay evidence. |
| Server-only config | `OPENAI_IDEA_MODEL`, `OPENAI_MODEL`, `ENFORCE_CREDIT_BUILD_PASS`, `STRIPE_PRO_MONTHLY_PRICE_ID`, `STRIPE_TEAM_MONTHLY_PRICE_ID` | Optional model names, deployment toggles, and non-secret price identifiers only. Do not store provider credentials here. Set `ENFORCE_CREDIT_BUILD_PASS=1` only after the credit migration and smoke-account entitlement policy are ready. |
...
Validation keywords: `public_client_boundary`, `server_only_secret_boundary`, `smoke_only_local_vars`, `service_role_server_only`, `NEXT_PUBLIC_only_browser_safe`, `stripe_secret_server_only`.
...
| Build-sync token smoke | `pnpm smoke:build-sync` | Login plus disposable idea when browser Supabase config is available | Verifies named external-tool token issue/use/revoke and STEP 7/8 connector UI. If `ENFORCE_CREDIT_BUILD_PASS=1` is enabled, run with `BUILD_SYNC_SMOKE_ALLOW_BUILD_PASS_SPEND=1` only for a disposable smoke account because it spends one 30-credit build pass on the disposable idea; the runner now preflights the authenticated credit summary and reports the credit shortfall before attempting a spend. |
