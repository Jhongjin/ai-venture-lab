...
| --- | --- | --- |
| Public client config | `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `NEXT_PUBLIC_APP_URL`, `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` | Browser-safe config. The anon key relies on RLS and is not a service-role key. Stripe publishable keys are not checkout authority and are future setup only while checkout is paused. |
| Server-only secrets | `SUPABASE_SERVICE_ROLE_KEY`, `TELEMETRY_INGEST_SECRET`, `OPENAI_API_KEY`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET` | Trusted server environments only. Never expose to browser bundles, screenshots, artifacts, chat, MCP packets, or Build Relay evidence. Stripe secrets are not needed until payment code resumes. |
| Server-only config | `OPENAI_IDEA_MODEL`, `OPENAI_MODEL`, `ENFORCE_CREDIT_BUILD_PASS`, `STRIPE_PRO_MONTHLY_PRICE_ID`, `STRIPE_TEAM_MONTHLY_PRICE_ID` | Optional model names, deployment toggles, and non-secret price identifiers only. Do not store provider credentials here. Stripe price IDs are future setup only while checkout is paused. Set `ENFORCE_CREDIT_BUILD_PASS=1` only after the credit migration and smoke-account entitlement policy are ready. |
| Smoke-only local vars | `SMOKE_URL`, `ROUTE_SMOKE_URL`, `BILLING_SMOKE_URL`, `BILLING_SMOKE_TIMEOUT_MS`, `BILLING_SMOKE_EMAIL`, `BILLING_SMOKE_PASSWORD`, `BILLING_SMOKE_HEADLESS`, `BILLING_SMOKE_ALLOW_AUTH_GRANT`, `BUILD_SYNC_SMOKE_URL`, `BUILD_SYNC_SMOKE_EMAIL`, `BUILD_SYNC_SMOKE_PASSWORD`, `BUILD_SYNC_SMOKE_HEADLESS`, `BUILD_SYNC_SMOKE_TIMEOUT_MS`, `BUILD_SYNC_SMOKE_KEEP_DATA`, `BUILD_SYNC_SMOKE_ALLOW_PROGRESS_WRITE`, `BUILD_SYNC_SMOKE_ALLOW_BUILD_PASS_SPEND`, `MARKET_SCAN_SMOKE_URL`, `MARKET_SCAN_SMOKE_TIMEOUT_MS`, `MARKET_SCAN_SMOKE_ALLOW_ESTIMATE`, `TELEMETRY_SMOKE_URL`, `TELEMETRY_SMOKE_IDEA_ID`, `BROWSER_SMOKE_URL`, `BROWSER_SMOKE_EMAIL`, `BROWSER_SMOKE_PASSWORD`, `BROWSER_SMOKE_ALLOW_WRITE`, `BROWSER_SMOKE_ALLOW_WORKSPACE_CREATE`, `BROWSER_SMOKE_HEADLESS`, `BROWSER_SMOKE_TIMEOUT_MS`, `BROWSER_SMOKE_SCREENSHOT`, `BROWSER_RLS_SMOKE_URL`, `BROWSER_RLS_SMOKE_EMAIL_A`, `BROWSER_RLS_SMOKE_PASSWORD_A`, `BROWSER_RLS_SMOKE_WORKSPACE_A_LABEL`, `BROWSER_RLS_SMOKE_EMAIL_B`, `BROWSER_RLS_SMOKE_PASSWORD_B`, `BROWSER_RLS_SMOKE_WORKSPACE_B_LABEL`, `BROWSER_RLS_SMOKE_HEADLESS`, `BROWSER_RLS_SMOKE_TIMEOUT_MS`, `BROWSER_RLS_SMOKE_EXPECT_BLOCKED`, `BROWSER_RLS_SMOKE_SCREENSHOT` | Operator terminal only. Do not commit values or copy them into docs, screenshots, artifacts, or relay packets. |

Validation keywords: `public_client_boundary`, `server_only_secret_boundary`, `smoke_only_local_vars`, `service_role_server_only`, `NEXT_PUBLIC_only_browser_safe`, `stripe_secret_server_only`, `stripe_checkout_paused_future_setup_only`.
...
| Billing credit smoke | `pnpm smoke:billing` | Anonymous negative-path check by default | Confirms credit summary and build-pass unlock reject unauthenticated requests. With `BILLING_SMOKE_ALLOW_AUTH_GRANT=1`, logs in and confirms credit summary readiness without spending a build pass; this may create the idempotent monthly Free grant. |
...
| Build-sync token smoke | `pnpm smoke:build-sync` | Login plus disposable idea when browser Supabase config is available | Without spend approval, verifies negative paths and treats HTTP 402 build-pass enforcement as a passed credit-gate smoke. Full named-tool token issue/use/revoke and STEP 7/8 connector UI coverage needs a pre-unlocked smoke idea or `BUILD_SYNC_SMOKE_ALLOW_BUILD_PASS_SPEND=1`, which spends one 30-credit build pass on a disposable idea after credit preflight. |
