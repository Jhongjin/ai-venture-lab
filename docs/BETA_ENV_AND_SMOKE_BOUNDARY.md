...
| Server-only config | `OPENAI_IDEA_MODEL`, `OPENAI_MODEL`, `ENFORCE_CREDIT_BUILD_PASS` | Optional model names and deployment toggles only. Do not store provider credentials here. Set `ENFORCE_CREDIT_BUILD_PASS=1` only after the credit migration and smoke-account entitlement policy are ready. |
