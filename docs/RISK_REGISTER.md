...
| Missing denied-case RLS evidence | Security/Operations | High | Use `docs/RLS_ALLOWED_DENIED_SMOKE_PLAN.md`; require a second disposable account and disposable workspace pair, record summary-only allowed/denied evidence, and block private-read beta claims if cross-workspace denied checks are missing | Open |
