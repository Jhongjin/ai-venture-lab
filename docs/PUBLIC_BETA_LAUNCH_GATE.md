...
- App build compatibility passed on Node 20 and Node 24 in CI, but this is not a production runtime migration. `engines.node` and Vercel Project Settings remain unchanged by this gate.
...
Validation keywords: `github_actions_non_blocking_for_controlled_beta`, `ci_app_node_matrix_20_24_configured`, `ci_app_node_matrix_passed`, `no_runtime_selection_change`, `openai_key_optional_with_local_fallback`, `cleanup_automation_not_required`, `rls_fixture_retention_allowed`.
