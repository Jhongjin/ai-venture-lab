...
5. 브라우저 다운로드 폴더에 있는 파일을 실제 개발할 Cursor 프로젝트 루트로 옮긴다. AI Venture Lab 프로젝트 폴더에서는 실행하지 않는다.
6. Cursor에서 해당 프로젝트 폴더를 연다. 현재 터미널이 다운로드 폴더나 AI Venture Lab 프로젝트라면 먼저 실제 프로젝트 루트로 이동한다.
