...
| --- | --- | --- | --- | --- |
| 2026-05-25 | Added setup-file download assertions to the external connector smoke | `c3df5df` | Script-only; pushed to `main` pending | `node --check scripts/smoke_build_sync_tokens.mjs`, `pnpm smoke:build-sync`; smoke now verifies the downloaded `.ps1` setup files for Cursor, Codex, Claude Code, and Google Antigravity include the expected project-root files |
