...
| --- | --- | --- | --- | --- |
| 2026-05-25 | Made external progress backup messages follow the selected build tool | Current commit | Pending push and production smoke | `pnpm lint`, `pnpm typecheck`, `pnpm release:check`, `pnpm quality:full`; backup progress import now names Cursor, Codex, Claude Code, or Google Antigravity instead of falling back to Cursor-only text |
