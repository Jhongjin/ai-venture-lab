...
| --- | --- | --- | --- | --- |
| 2026-05-26 | Clarified first Cursor terminal command and guarded downloaded connector CLI syntax | Current commit | Push to `main`, then production smoke after deployment | `node --check scripts/smoke_build_sync_tokens.mjs`, `pnpm typecheck`, `pnpm release:check`, `pnpm quality:full`; pre-deploy `pnpm smoke:build-sync` reached old production UI and is expected to pass after deployment |
