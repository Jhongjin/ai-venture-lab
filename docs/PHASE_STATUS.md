...
| --- | --- | --- | --- | --- |
| 2026-05-25 | Added invalid build-sync token TTL denial smoke | Current commit | Script/docs only; pushed to `main` after verification | `node --check scripts/smoke_build_sync_tokens.mjs`, `pnpm release:check`, `pnpm smoke:build-sync`; smoke now confirms negative `expiresInSeconds` token requests return HTTP 400 before any connection is issued |
| 2026-05-25 | Added browser smoke coverage for STEP 6 guide sequencing | Current commit | Script-only; pushed to `main` after verification | `node --check scripts/smoke_browser.mjs`, `pnpm release:check`, `pnpm smoke:browser`; browser smoke now protects the `/guide` wording that STEP 6 work-order review comes before STEP 7 external connection files |
