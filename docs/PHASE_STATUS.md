...
| --- | --- | --- | --- | --- |
| 2026-05-25 | Simplified STEP 1 first-run AI actions by showing only context-relevant buttons | Current commit | User-facing STEP 1 copy/control change; production deploy required after commit | `pnpm lint`, `pnpm typecheck`, `pnpm release:check`, `pnpm quality:full`; empty input now shows one AI idea-discovery action, pasted input shows one AI organize action, and candidate refresh appears only after generated candidates exist |
