...
| 2026-05-22 | Hardened telemetry smoke env loading and checked rerun readiness | Current commit | Blocked before telemetry write; no app deploy or production mutation | Telemetry smoke now loads local telemetry env names and sends full single-event names; `pnpm smoke:telemetry` reached production but returned HTTP 401 because the local telemetry secret was rejected; no secret values read back or recorded |
...
| Authenticated telemetry smoke execution | Blocked | 2026-05-22 approved rerun reached the production ingest endpoint, but the local `TELEMETRY_INGEST_SECRET` value was rejected with HTTP 401; no secret values were read back or recorded | Set the exact current Production `TELEMETRY_INGEST_SECRET` in local smoke-only env, ensure production is redeployed if the Vercel value changed, then rerun `pnpm smoke:telemetry` and `pnpm smoke:telemetry:funnel` |
...
Learning telemetry writes have passed post-rotation smoke with a disposable idea id. The 2026-05-22 approved rerun is blocked because the local telemetry secret was rejected by the production ingest endpoint. Reconfirm `telemetry_events` table posture and RLS policies only when telemetry migrations, endpoint behavior, or production environment settings change.
