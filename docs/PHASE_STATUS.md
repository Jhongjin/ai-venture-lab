...
| --- | --- | --- | --- | --- |
| 2026-05-25 | Kept result type separate from build delivery across downstream packages | Current commit | User-facing package/docs change; production deploy required after commit | `pnpm lint`, `pnpm typecheck`, `pnpm release:check`, `pnpm quality:full`, `pnpm smoke:build-sync`; manual intake now stores build-delivery preference and final packages use the selected external tool rather than a default Cursor path |
