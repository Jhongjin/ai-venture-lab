...
| Authenticated telemetry smoke execution | Blocked | 2026-05-22 approved rerun was attempted, but the current local terminal did not have runnable `TELEMETRY_INGEST_SECRET` and `TELEMETRY_SMOKE_IDEA_ID` values; no secret values were read back or recorded | Add the local smoke-only values, then rerun `pnpm smoke:telemetry` and `pnpm smoke:telemetry:funnel` |
...
Learning telemetry writes have passed post-rotation smoke with a disposable idea id. The 2026-05-22 approved rerun is blocked until the local terminal has runnable `TELEMETRY_INGEST_SECRET` and `TELEMETRY_SMOKE_IDEA_ID` values. Reconfirm `telemetry_events` table posture and RLS policies only when telemetry migrations, endpoint behavior, or production environment settings change.
