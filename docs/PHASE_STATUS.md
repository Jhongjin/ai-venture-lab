...
| --- | --- | --- | --- | --- |
| 2026-05-25 | Updated the public guide from Cursor-only wording to the four supported external tools | Current commit | Pending commit/deploy | `node --check scripts/smoke_browser.mjs`, `pnpm lint`, `pnpm typecheck`, `pnpm release:check`, `pnpm quality:full`; guide now describes Cursor, Codex, Claude Code, and Google Antigravity as named connection choices |
