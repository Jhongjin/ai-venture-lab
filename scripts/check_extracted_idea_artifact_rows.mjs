...
assert.equal(insertRow.name, "AI Venture Lab");
assert.equal(insertRow.product_surface, "web_app");
assert.equal(insertRow.stage, "research");
assert.equal(insertRow.decision, "research_more");
assert.equal(insertRow.organization_id, "org-1");
assert.equal(insertRow.mvp_speed, 80);
assert.equal(insertRow.differentiation, 72);
assert.match(insertRow.signal, /핵심 가설\n- 사용자는 정리보다 확인을 원합니다\./);
assert.match(insertRow.risk_summary, /리스크 등급: 보통/);
