...
const extractActionPattern =
  /이 내용으로 아이디어 정리하기|킵한 후보로 아이디어 정리하기|이 후보들 중 하나로 정리하기|AI가 아이디어 도출하기|AI로 아이디어 구체화|AI로 후보 찾기|AI 후보 발굴/;
...
async function openExtractTask(page) {
  ...
  const focusCta = page.getByRole("button", {
    name: /이 내용으로 아이디어 정리하기|킵한 후보로 아이디어 정리하기|이 후보들 중 하나로 정리하기|AI가 아이디어 도출하기|아이디어 구체화|후보 찾기/,
  });
