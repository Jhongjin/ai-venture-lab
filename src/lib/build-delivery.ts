...
export const externalBuildToolProfiles: Record<ExternalBuildToolKey, ExternalBuildToolProfile> = {
  ...
  codex: {
    ...
    startMethod: "연결 파일을 프로젝트 루트에서 실행한 뒤 Codex 세션에 시작 지시문을 넣고 첫 작업부터 진행합니다.",
    automationLabel: "자동 동기화 지원",
    ...
    handoffSteps: [
      ...
      "Codex 작업 세션에서 같은 프로젝트를 열고 AI_VENTURE_CODEX_START.md를 첫 메시지로 넣습니다.",
      "T-001부터 한 번에 하나씩 진행하고, 완료 후 .codex/venture-lab-cli.mjs record-progress 명령으로 결과를 남깁니다.",
  ...
  claude_code: {
    ...
    startMethod: "연결 파일을 프로젝트 루트에서 실행한 뒤 Claude Code에서 MCP 상태를 확인하고 첫 작업부터 진행합니다.",
    automationLabel: "자동 동기화 지원",
    ...
    handoffSteps: [
      ...
      "받은 PowerShell 파일을 실제 개발할 프로젝트 루트에 둡니다.",
      "프로젝트 루트에서 화면의 PowerShell 명령을 실행합니다.",
      "Claude Code를 같은 프로젝트 루트에서 열고 /mcp에서 ai-venture-lab 연결을 확인합니다.",
      "AI_VENTURE_CLAUDE_START.md를 첫 메시지로 넣고 T-001부터 진행합니다.",
  ...
  antigravity: {
    ...
    startMethod: "연결 파일을 프로젝트 루트에서 실행한 뒤 Antigravity Agent에 시작 지시문을 넣고 첫 수직 슬라이스만 진행합니다.",
    automationLabel: "자동 동기화 지원",
    ...
    handoffSteps: [
      ...
      "받은 PowerShell 파일을 실제 개발할 프로젝트 루트에 둡니다.",
      "프로젝트 루트에서 화면의 PowerShell 명령을 실행합니다.",
      "Antigravity에서 같은 프로젝트를 열고 프로젝트 지침과 MCP 설정 파일을 확인합니다.",
      "AI_VENTURE_ANTIGRAVITY_START.md를 Agent 첫 메시지로 넣고 T-001부터 진행합니다.",
