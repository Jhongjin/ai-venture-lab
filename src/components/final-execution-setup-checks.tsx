...
export function FinalExecutionSetupChecks({ progressPath, startFileName }: FinalExecutionSetupChecksProps) {
  ...
  const recoveryItems = [
    ...
    ["먼저 볼 것", "기존 앱은 package.json, app 또는 src 확인"],
  ...
  return (
    <>
      ...
      <div data-smoke="final-execution-root-check" className="border border-slate-200 bg-slate-50 p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="grid gap-2 text-sm leading-6 text-slate-700 sm:min-w-[420px] sm:grid-cols-3">
            ...
            <div>
              ...
              <div className="mt-1 font-semibold text-slate-950">기존 앱 파일 또는 새 빈 폴더</div>
            </div>
          </div>
        </div>
        <div
        ...
        >
          실행 전 5초 확인: 기존 앱이면 지금 터미널 폴더에 package.json, app 또는 src가 보이는지 확인합니다. 새 앱이면
          방금 만든 빈 폴더여도 됩니다. 현재 폴더가 다운로드 폴더나 AI Venture Lab 프로젝트라면 새 앱 프로젝트로 이동한 뒤
          실행하세요.
        </div>
