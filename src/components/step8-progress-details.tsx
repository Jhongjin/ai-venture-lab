...
import {
  ...
  type Step8ProgressDisplayItem,
} from "@/lib/step8-learning-summary";

export type Step8ProgressItem = Step8ProgressDisplayItem;
...
export function Step8ProgressDetails({ items }: Step8ProgressDetailsProps) {
  const detailsState = buildStep8ProgressDetailsState(items);

  return (
    <details data-smoke="step8-progress-details" className="mt-4 border border-slate-200 bg-slate-50 p-3">
      <summary className="cursor-pointer list-none text-sm font-semibold text-slate-950">
        {detailsState.summaryLabel}
      </summary>
      <div className="mt-3 grid gap-2">
        {detailsState.hasItems ? (
          detailsState.items.map((item) => (
            <div key={item.id} className={`border p-3 ${item.itemClassName}`}>
              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {item.showNextBadge ? <span className="avl-pill avl-pill-info">다음 작업</span> : null}
                  </div>
                  <p className="mt-1 text-sm leading-6 text-slate-600">{item.statusDetail}</p>
                  {item.missingEvidenceText ? (
                    <p className="mt-1 text-xs leading-5 text-slate-500">{item.missingEvidenceText}</p>
                  ) : null}
                </div>
                <div className="flex shrink-0 flex-wrap gap-2 sm:justify-end">
                  <span className={item.statusTone}>{item.statusLabel}</span>
                  <span className="avl-pill avl-pill-neutral">{item.evidenceLabel}</span>
                </div>
              </div>
            </div>
          ))
