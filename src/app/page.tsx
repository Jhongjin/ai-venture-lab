  ...
  return (
    <main className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
      <div className="mx-auto grid w-full max-w-[1440px] gap-4 px-4 py-4 sm:px-6">
        <header className="rounded-[12px] border border-slate-200 bg-white px-4 py-3 shadow-[0_1px_2px_rgba(15,23,42,0.04)] sm:px-5">
          <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
            <div className="min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <h1 className="text-[16px] font-semibold tracking-tight text-slate-950 sm:text-[18px]">
                  아이디어 실행 보드
                </h1>
              ...
              <p className="mt-1.5 max-w-2xl text-[12px] leading-6 text-slate-600">
            ...
            <div className="flex flex-wrap gap-2 xl:justify-end">
              {headerStats.map(([label, value]) => (
                <div key={label} className="min-w-[92px] rounded-full border border-slate-200 bg-slate-50 px-3 py-2">
                  <div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-500">{label}</div>
                  <div className="mt-0.5 text-[15px] font-semibold text-slate-950">{value}</div>
                </div>
              ))}
            </div>
          </div>
