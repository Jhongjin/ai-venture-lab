  ...
  return (
    <main className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
      <div className="mx-auto grid w-full max-w-[1440px] gap-4 px-4 py-4 sm:px-6">
        <header className="rounded-[12px] border border-slate-200 bg-white px-4 py-4 sm:px-6">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
            <div className="min-w-0">
              <div className="mb-2 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.24em] text-slate-500">
                <FlaskConical size={15} />
                AI Venture Lab
              </div>
              <div className="flex flex-wrap items-center gap-2">
                <h1 className="text-[22px] font-semibold tracking-tight text-slate-950 sm:text-[28px]">
                  아이디어 실행 보드
                </h1>
              ...
              <p className="mt-2 max-w-3xl text-[13px] leading-6 text-slate-600">
                아이디어를 발굴하고 검증한 뒤, 바로 기획과 실행 패키지까지 이어가는 단일 작업 공간입니다.
              </p>
            </div>

            <div className="grid grid-cols-2 gap-2 xl:min-w-[360px] xl:grid-cols-4">
              {headerStats.map(([label, value]) => (
                <div key={label} className="rounded-[10px] border border-slate-200 bg-slate-50 px-3 py-2.5">
